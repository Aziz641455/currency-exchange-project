...
def calculate_exchange_rate_change_part_b():
    try:
        ...
        # Calculate the percentage change for 300 currency pairs
        for latest, previous in zip(latest_rates, previous_rates):
            ...
            # Insert into exchange_rates_part_b table
            cur.execute("""
                INSERT INTO exchange_rates_part_b (pair, rate, pct_change, timestamp)
                VALUES (%s, %s, %s, %s)
...
with DAG(
    dag_id='part_b_currency_exchange_dag',
    ...
    task1 = PythonOperator(
        task_id='calculate_exchange_rate_change_part_b',
        python_callable=calculate_exchange_rate_change_part_b
    )
